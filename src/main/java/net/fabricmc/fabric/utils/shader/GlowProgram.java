...
public class GlowProgram {
    ...
    private Uniform1f softness;
    private Uniform4f color1;
    private Uniform4f color2;
    private Uniform4f color3;
    private Uniform4f color4;
    public static final ManagedCoreShader GLOW = ShaderEffectManager.getInstance().manageCoreShader(Identifier.of((String)"tulip", (String)"glow"), VertexFormats.POSITION);

    public GlowProgram() {
        this.setup();
    }

    public void setParameters(float x, float y, float width, float height, float r, float soft, Color c1, Color c2, Color c3, Color c4) {
        int scale = (Integer)ClientMain.mc.options.getGuiScale().getValue();
        this.radius.set(r * (float)scale);
        this.softness.set(soft * (float)scale);
    ...
    protected void setup() {
        this.uSize = GLOW.findUniform2f("uSize");
        this.uLocation = GLOW.findUniform2f("uLocation");
        this.radius = GLOW.findUniform1f("radius");
        this.softness = GLOW.findUniform1f("softness");
        this.color1 = GLOW.findUniform4f("color1");
        this.color2 = GLOW.findUniform4f("color2");
        this.color3 = GLOW.findUniform4f("color3");
        this.color4 = GLOW.findUniform4f("color4");
    }
}
